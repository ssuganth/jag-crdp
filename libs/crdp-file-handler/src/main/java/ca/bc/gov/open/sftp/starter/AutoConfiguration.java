...
public class AutoConfiguration {
    ...
    public AutoConfiguration(SftpProperties sftpProperties) {
        ...
        logger.debug(
                "SFTP Configuration: Remote Directory => [{}]",
                this.sftpProperties.getRemoteLocation());
        logger.debug(
                "SFTP Configuration: Known Host File => [{}]",
                this.sftpProperties.getKnownHostsFileName());
    }
    ...
    public JSch sftpSession(SftpProperties sftpProperties) throws JSchException {
        ...
        if (StringUtils.isNotBlank(sftpProperties.getSshPrivateKey())) {
            if (StringUtils.isNotBlank(sftpProperties.getSshPrivatePassphrase())) {
                jsch.addIdentity(
                        sftpProperties.getSshPrivateKey(),
                        sftpProperties.getSshPrivatePassphrase());
            } else {
